...
async def manage_subs(callback: CallbackQuery, session: AsyncSession):
    ...
    if vip_channel_id:
        try:
            chat = await callback.bot.get_chat(vip_channel_id)
            channel_name = chat.title or str(vip_channel_id)
        except Exception:
            channel_name = str(vip_channel_id)
        text += f"\nCanal VIP: {channel_name}"
...
@router.callback_query(F.data.regexp(r"^vip_edit_\d+$"))
